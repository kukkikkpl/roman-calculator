export function romanToInt(roman: string) {
  ...
  const romanReverse = roman.split("").reverse();
  Object.keys(romanReverse).map(char => {
    const character = romanReverse[char]
    current = romanNumerals[character] || 0;
    if (current >= previous) {
      result += current;
    } else {
      result -= current;
...
export function intToRoman(num: number) {
  ...
  Object.keys(romanNumerals).map(roman => {
    const value = romanNumerals[roman]
